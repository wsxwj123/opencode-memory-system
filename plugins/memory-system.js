...
export const MemorySystemPlugin = ({ client }) => {
  ...
  function buildDashboardHtmlLegacy(data) {
    ...
    '    </main>',
      ...
      '    }',
      '    async function apiPost(url,payload){ const r=await apiFetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}); if(!r.ok) throw new Error(await r.text()); return r.json(); }',

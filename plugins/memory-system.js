...
export const MemorySystemPlugin = ({ client }) => {
  ...
  function buildDashboardHtml(data) {
    ...
    const html = [
      ...
      '    </aside>',
      '    <main class="main">',
      '      <div class="panel"><div class="tabbar"><button id="tabSessionsBtn" class="tab-btn active">会话页</button><button id="tabTemplateBtn" class="tab-btn">模板设置</button><button id="tabLlmBtn" class="tab-btn">LLM设置</button><button id="tabSettingsBtn" class="tab-btn">参数页</button><button id="tabTrashBtn" class="tab-btn">回收站</button></div></div>',
      ...
      '        <div class="panel"><details id="globalPrefsFold" class="fold" open><summary id="globalPrefsFoldSummary">全局偏好设置</summary><div style="margin-top:8px;"><h1 id="globalTitle" style="font-size:16px;">Global Preferences</h1><div class="sub" id="tokenHint">Token estimate is approximate (chars/4).</div><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;"><label id="pretrimProfileLabel" for="pretrimProfileSel" style="font-size:12px;color:var(--muted);">Pretrim Profile</label><select id="pretrimProfileSel"><option value="conservative">Conservative (~20%, preserve detail)</option><option value="balanced" selected>Balanced (~40%, recommended)</option><option value="aggressive">Aggressive (~60%, strongest trim)</option></select><button id="savePretrimProfileBtn" style="height:30px;">Save</button><span id="pretrimProfileHint" class="sub" style="margin:0;"></span></div><div id="globalPrefs" class="empty">No global preferences.</div></div></details></div>',
        '        <div class="panel"><h1 id="settingsTitle" style="font-size:16px;">Memory System Settings</h1><div class="sub" id="settingsHint">Adjust runtime behavior. Saved locally and persisted.</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px;"><details id="toggleFold" class="fold" open><summary id="toggleFoldSummary">开关参数（默认展开）</summary><div id="settingsToggleForm" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:center;margin-top:8px;"></div></details><details id="numericFold" class="fold"><summary id="numericFoldSummary">数值参数（默认折叠）</summary><div id="settingsNumericForm" style="display:grid;grid-template-columns:1fr 1fr;gap:8px;align-items:center;margin-top:8px;"></div></details></div><div style="margin-top:10px;display:flex;gap:8px;align-items:center;"><button id="settingsSaveBtn" style="height:30px;">Save Settings</button><span id="settingsStatus" class="sub" style="margin:0;"></span></div></div>',
        '        ',
      '      </section>',
    ...
    '    </main>',
      ...
      '    I18N.zh.numericFoldSummary = "数值参数（默认折叠）"; I18N.en.numericFoldSummary = "Numeric Params (default collapsed)";',

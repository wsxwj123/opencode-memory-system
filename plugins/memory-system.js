...
export const MemorySystemPlugin = ({ client }) => {
  ...
  const AUTO_SYSTEM_PROMPT_AUDIT_ENABLED = false;
  ...
  function buildDashboardHtmlLegacy(data) {
    ...
    '    </main>',
      ...
      '      { key:"systemPromptAuditEnabled", type:"bool", default:false, labelZh:"保存系统层审计", labelEn:"Store system-layer audit" },',
      ...
      '      systemPromptAuditEnabled:{zh:"开启后，插件会把最近一次真正发给模型的系统层文本保存下来，供 37777 页面排查到底是 MCP、skill 还是插件在干扰。默认关闭，避免额外存档。",en:"Store the last real system-layer text sent to the model for dashboard debugging."},',
      ...
      '    I18N.zh.systemAuditEmpty = "暂无系统层审计记录"; I18N.en.systemAuditEmpty = "no system-layer audit captured yet";',
    ...
    rendered = rendered.replace(
      ...
      'if(s.summary&&s.summary.compressedText){ const summary=document.createElement("div"); summary.className="ev"; const meta=document.createElement("div"); meta.className="meta"; const reason=(s.budget&&s.budget.lastCompactionReason)?(" · "+s.budget.lastCompactionReason):""; const paInfo=s.pruneAudit?(` · prune(last:${s.pruneAudit.lastSource||"-"}, d=${s.pruneAudit.lastDiscardRemoved||0}, e=${s.pruneAudit.lastExtractMoved||0})`):""; meta.textContent="compressed summary"+reason+paInfo; const txt=document.createElement("div"); txt.className="txt"; txt.textContent=s.summary.compressedText; summary.appendChild(meta); summary.appendChild(txt); events.appendChild(summary); } { const sys=document.createElement("div"); sys.className="ev"; const sm=document.createElement("div"); sm.className="meta"; sm.textContent=(typeof t==="function"?t("systemAuditCard"):"send-time system audit")+` · tokens~${s.systemPrompt&&s.systemPrompt.lastObservedTokens||0} · chars:${s.systemPrompt&&s.systemPrompt.lastObservedChars||0} · lines:${s.systemPrompt&&s.systemPrompt.lastObservedLines||0} · model:${s.systemPrompt&&s.systemPrompt.lastObservedModel||"-"}`; const stxt=document.createElement("div"); stxt.className="txt"; stxt.textContent=(s.systemPrompt&&s.systemPrompt.lastObservedText)||((typeof t==="function"?t("systemAuditEmpty"):"no system-layer audit captured yet")); sys.appendChild(sm); sys.appendChild(stxt); events.appendChild(sys); }'
    );
    return rendered;
  }

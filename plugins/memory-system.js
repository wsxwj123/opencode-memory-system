...
export const MemorySystemPlugin = ({ client }) => {
  ...
  function buildDashboardHtml(data) {
    ...
    const html = [
      ...
      '    </aside>',
      '    <main class="main">',
      '      <div class="panel"><div class="tabbar"><button id="tabSessionsBtn" class="tab-btn active">会话页</button><button id="tabSettingsBtn" class="tab-btn">参数页</button><button id="tabLlmBtn" class="tab-btn">LLM设置</button><button id="tabTrashBtn" class="tab-btn">回收站</button></div></div>',
    ...
    '    </main>',
      ...
      '    async function fetchLlmModels(){ try{ const provider=String(($("llm_independentLlmProvider")&&$("llm_independentLlmProvider").value)||"openai_compatible"); const baseURL=String(($("llm_independentLlmBaseURL")&&$("llm_independentLlmBaseURL").value)||"").trim(); const apiKey=String(($("llm_independentLlmApiKey")&&$("llm_independentLlmApiKey").value)||"").trim(); const timeoutMs=Number(($("llm_independentLlmTimeoutMs")&&$("llm_independentLlmTimeoutMs").value)||12000); const r=await apiPost("/api/memory/llm/models",{provider,baseURL,apiKey,timeoutMs}); if(!r||!r.ok){ if(llmStatus) llmStatus.textContent=(LANG==="zh"?"获取模型失败: ":"Model fetch failed: ")+(r&&r.error?r.error:"unknown"); return; } const list=$("llmModelList"); if(list){ list.innerHTML=""; (Array.isArray(r.models)?r.models:[]).forEach((m)=>{ const o=document.createElement("option"); o.value=String(m||""); list.appendChild(o); }); } const modelInput=$("llm_independentLlmModel"); if(modelInput&&!String(modelInput.value||"").trim()&&Array.isArray(r.models)&&r.models.length) modelInput.value=String(r.models[0]); if(llmStatus) llmStatus.textContent=t("llmModelsLoaded")+": "+String((r&&r.count)||0); }catch(e){ if(llmStatus) llmStatus.textContent=(LANG==="zh"?"获取模型失败: ":"Model fetch failed: ")+String(e&&e.message?e.message:e); } }',
      '    async function validateLlmConfig(){ try{ const provider=String(($("llm_independentLlmProvider")&&$("llm_independentLlmProvider").value)||"openai_compatible"); const baseURL=String(($("llm_independentLlmBaseURL")&&$("llm_independentLlmBaseURL").value)||"").trim(); const apiKey=String(($("llm_independentLlmApiKey")&&$("llm_independentLlmApiKey").value)||"").trim(); const model=String(($("llm_independentLlmModel")&&$("llm_independentLlmModel").value)||"").trim(); const timeoutMs=Number(($("llm_independentLlmTimeoutMs")&&$("llm_independentLlmTimeoutMs").value)||12000); const r=await apiPost("/api/memory/llm/validate",{provider,baseURL,apiKey,model,timeoutMs}); if(!r||!r.ok){ if(llmStatus) llmStatus.textContent=(LANG==="zh"?"验证失败: ":"Validation failed: ")+(r&&r.error?r.error:"unknown"); return; } if(llmStatus) llmStatus.textContent=t("llmValidateOk"); }catch(e){ if(llmStatus) llmStatus.textContent=(LANG==="zh"?"验证失败: ":"Validation failed: ")+String(e&&e.message?e.message:e); } }',
